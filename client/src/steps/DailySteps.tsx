...
function DailySteps({fromDate, toDate, title = "Daily Steps", onDayClick, onDateChange}: DailyStepsProps) {
    ...
    const from = fromDate || "2024-01-01T00:00:00-05:00";
    const to = toDate || "2024-12-31T23:59:59-05:00";
    ...
    let chartData: Array<{ date: string, steps: number, day: number, isMonthStart: boolean, formattedDate: string }> = [];
    ...
    if (isSpecificDate && stepsQuery.data) {
        ...
        const sortedData = dailyStepsQuery.data.dailyStepsSum
            .map(item => {
                // safely build a local-time Date (yyyy, mm-1, dd)
                const [y, m, d] = item.date.split('-').map(Number);
                return {
                    date: item.date,
                    steps: item.totalSteps,
                    dateObj: new Date(y, m - 1, d)      // <-- stays in the right month
                };
            })
        ...
        // Process the sorted data to add formatted dates
        chartData = sortedData.map((item, index) => {
            ...
            // Always use month name
            const monthNames = ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec'];
            formattedDate = monthNames[month];

            // Set isMonthStart flag if this is the first day of a new month
                        ...
                        <ResponsiveContainer width="100%" height={400}>
                            <BarChart data={chartData} margin={{top: 20, right: 30, left: 20, bottom: 50}}>
                                ...
                                <XAxis
                                    ...
                                    angle={-45}
                                    textAnchor="end"
                                    ticks={monthTicks}   // ← only these labels are rendered
                                    tick={(props) => {
                                        const {x, y, payload} = props;
                                        // Check if this tick represents the start of a month
                                        const isMonthStart = payload.payload?.isMonthStart;
                                        ...
                                        return (
                                            <g transform={`translate(${x},${y})`}>
                                                <text
                                                    ...
                                                    dy={16}
                                                    textAnchor="end"
                                                    className={isMonthStart ? "month-label" : "day-label"}
                                                    transform="rotate(-45)"
                                                >
                                                    {payload.value}
                                                </text>
