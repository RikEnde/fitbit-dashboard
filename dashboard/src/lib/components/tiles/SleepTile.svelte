...
>
	<div class="flex items-center justify-between mb-3">
		<h3 class="text-sm font-medium text-theme-text-secondary uppercase tracking-wide">Sleep</h3>
		<svg
	...
	{:else}
		<div class="flex items-center gap-4">
			...
			<div class="flex-1">
				{#if totalSleep > 0}
					<p class="text-3xl font-bold text-theme-text">{formatDuration(totalSleep)}</p>
				...
				{:else}
					...
					<p class="text-sm text-theme-text-muted mt-1">No sleep recorded</p>
				{/if}
			</div>
		</div>
		...
		{#if totalStageMinutes > 0}
			<div class="mt-4">
				<p class="text-xs text-theme-text-muted mb-2">Sleep Stages</p>
		...
		<!-- Efficiency -->
		{#if efficiency > 0}
			<div class="mt-3 pt-3 border-t border-theme-border flex justify-between items-center">
				<span class="text-xs text-theme-text-muted">Sleep Efficiency</span>
